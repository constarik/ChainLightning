...
// Сохранение при аборте (Ctrl+C)
function saveAndExit() {
  console.log('\n\nAborted! Saving partial results...');
  const finalRTP = results.length > 0 ? (totalWin / (results.length * BET) * 100).toFixed(2) : '0.00';
  fs.writeFileSync(outputFile, JSON.stringify(results));
  console.log(`Saved ${results.length} seeds to ${outputFile} (RTP: ${finalRTP}%)`);
  process.exit(0);
}
...
while (results.length < count) {
  ...
  // Проверяем лимит частоты для этого win (кроме нулей)
  const currentWinCount = winCounts[win] || 0;
  const maxWinCount = Math.floor(count * maxWinPct);
  if (win !== 0 && currentWinCount >= maxWinCount) {
  ...
  if (take) {
    ...
    if (percent !== lastPercent && percent % 5 === 0) {
      ...
      process.stdout.write(`\r${percent}% | ${results.length}/${count} | RTP: ${rtp}% | tested: ${i}      `);
      lastPercent = percent;
    }
  } else {
